...
// Evaluar mergeEngine.js en el contexto global
const mergeEngineCode = fs.readFileSync(path.join(__dirname, '..', 'js', 'mergeEngine.js'), 'utf8');
eval(mergeEngineCode); // Define performMerge globalmente
...
// Buscar Edalidis y Maira en el resultado
console.log('--- BUSCANDO EN BASE (MAYO) ---');
baseData.forEach((row, i) => {
  if (row['TITULAR'] && (row['TITULAR'].includes('Edalidis') || row['TITULAR'].includes('Maira Alejandra Meza'))) {
    console.log(`Posición en Base ${i}:`, row['TITULAR'], 'EXQ:', row['EXQ'], 'Fila real:', row['__rowNum']);
  }
});

console.log('\n--- BUSCANDO EN MERGE (JUNIO) ---');
mergeData.forEach((row, i) => {
  if (row['TITULAR'] && (row['TITULAR'].includes('Edalidis') || row['TITULAR'].includes('Maira Alejandra Meza'))) {
    console.log(`Posición en Merge ${i}:`, row['TITULAR'], 'EXQ:', row['EXQ'], 'Fila real:', row['__rowNum']);
  }
});

console.log('\n--- BUSCANDO EN RESULTADO DEL MERGE ---');
result.mergedData.forEach((row, i) => {
  if (row['TITULAR'] && (row['TITULAR'].includes('Edalidis') || row['TITULAR'].includes('Maira Alejandra Meza'))) {
    console.log(`Posición en Resultado ${i}:`, row['TITULAR'], 'EXQ:', row['EXQ'], 'Fila real:', row['__rowNum']);
  }
});
